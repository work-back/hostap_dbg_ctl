...
/* LYJ Patch Begin */
static int hostapd_ctrl_iface_dbg_ctl(struct hostapd_data *hapd, char *cmd,
					char *buf, size_t buflen)
{
    return dbg_ctl_run(cmd, buf, buflen);
}
/* LYJ Patch End */
					      ...
					      socklen_t fromlen)
{
    ...
    wpa_printf(MSG_ERROR, "cmd buf: [%s]", buf);
...
/* LYJ Patch Begin */
	} else if (os_strncmp(buf, "DBG_CTL", 7) == 0) {
		reply_len = hostapd_ctrl_iface_dbg_ctl(
			hapd, buf + 7, reply, reply_size);
/* LYJ Patch End */
#ifdef NEED_AP_MLME
